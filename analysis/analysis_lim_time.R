...
data<-readRDS("data_lim_time.RDS")
...
polys<-4
varlist<-c("homogamy","mults_gr_1","connected","mults","n","char_ratio", "sex_ratio","homophily","s_p_max","s_p_median","g_density","clustering","max_time")
vars_power<-paste0("I(",varlist[c(-1,-2,-3)],"^",mapply(rep,c(2:polys),MoreArgs = list(times=length(varlist[c(-1,-2,-3)]))),")")
