...
simulation<-function(run){
  ...
  search_times<-rtruncnorm(data[run,"n"],a=0,b=Inf,mean=char*data[run,"mults"],sd=data[run,"stand_dev"])
  #search_times<-char*data[run,"mults"]+rexp(data[run,"n"],data[run,"decay"]) #vector with time players' begin searching
